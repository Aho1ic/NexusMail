...
- Docker Hub：`docker.io/aho1ic901/nexusmail`（需配置下述变量与 secret，未配置时自动跳过，只发 GHCR）
...
推送镜像前工作流会先跑 `test` job（Go 全量测试带 `sqlite_fts5` + 前端 Vitest）：`build` 直接 `needs` 它，唯一移动标签的 `merge` 又 `needs` `build`，所以标签不会指向测试失败的提交。工作流还按 ref 串行（`concurrency`，不取消进行中的 run），避免两次推送的 merge 交错后 `latest` 停在较旧的提交上。
...
Docker Hub 需要一个 Actions 变量和一个 secret。用户名必须是**变量**而不是 secret：工作流在 `prepare` 里把镜像列表作为 job output 传给 `build`，而 Actions 拒绝跨 job 传递含 secret 的 output，用 secret 会让 `build` 拿到空列表并失败。

```bash
gh variable set DOCKERHUB_USERNAME --body '<你的 Docker Hub 用户名>'
...
```bash
make dev        # 运行 Go 服务
make test       # Go 全量测试（启用 sqlite_fts5）+ 前端 Vitest
make test-race  # race detector
make web-build  # 类型检查并构建 SPA，清空后重写嵌入资源
