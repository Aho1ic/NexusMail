  ...
  return <>
    ...
    <article className="flex-1 overflow-y-auto px-6 py-8 lg:px-12 xl:px-16">
      <div className="mx-auto max-w-3xl"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-pine/40">{formatFullDate(message.received_at)}</p><h1 className="mt-3 font-serif text-3xl leading-tight lg:text-4xl">{message.subject || '（无主题）'}</h1>
        ...
        {details && message.body_state === 'error' && <div className="my-10 rounded-card bg-amber-50 p-5 text-sm text-amber-800 shadow-lift-1">正文获取失败，下次账号同步时会自动重试。</div>}
        {details && message.body_state !== 'ready' && message.body_state !== 'error' && <div className="my-10 rounded-card bg-sage/50 p-5 text-sm text-pine shadow-lift-1">正文正在从邮件服务商异步获取，稍后会自动刷新。</div>}
