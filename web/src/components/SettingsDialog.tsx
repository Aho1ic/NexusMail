  ...
  return <Dialog label="设置" onClose={onClose} className="flex max-h-[min(88vh,780px)] w-[min(94vw,540px)] flex-col overflow-hidden rounded-panel bg-white shadow-glass-high">
      ...
      <div className="flex-1 overflow-y-auto px-7 py-2">
        <SettingsSection icon={<Bell size={14} />} title="通知">
          ...
          {permission === 'default' && <div className="flex items-center justify-between gap-3 rounded-card bg-sage/50 px-3.5 py-3 text-xs text-pine"><span>浏览器尚未授权通知。</span><button onClick={askPermission} disabled={asking} className="button-secondary shrink-0">{asking ? '请求中…' : '授权'}</button></div>}
          {permission === 'denied' && <p className="rounded-card bg-amber-50 px-3.5 py-3 text-xs leading-5 text-amber-800">浏览器已拒绝通知权限，需在地址栏的站点设置中重新允许后才会生效。</p>}
          {permission === 'unsupported' && <p className="rounded-card bg-black/[.03] px-3.5 py-3 text-xs leading-5 text-black/45">当前浏览器或非 HTTPS 环境不支持桌面通知。</p>}
        </SettingsSection>
        ...
        <SettingsSection icon={<Keyboard size={14} />} title="快捷键">
          ...
          <div className={`grid grid-cols-2 gap-2 text-xs ${preferences.keyboardShortcuts ? 'text-black/55' : 'text-black/25'}`}>
            {[['J', '下一封'], ['K', '上一封'], ['C', '写邮件'], ['E', '归档当前邮件']].map(([key, label]) => <div key={key} className="flex items-center gap-2 rounded-2xl bg-paper/70 px-3 py-2"><kbd>{key}</kbd>{label}</div>)}
          </div>
        </SettingsSection>

        <SettingsSection icon={<AtSign size={14} />} title="账户">
          {accounts.length === 0 && <p className="rounded-card bg-black/[.03] px-3.5 py-3 text-xs text-black/45">还没有连接任何邮箱。</p>}
          {accounts.map(account => <div key={account.id} className="rounded-card border border-black/5 bg-paper/60 p-3.5 shadow-lift-1">
