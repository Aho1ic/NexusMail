  ...
  return <aside className={`${visible ? 'flex' : 'hidden'} md:flex pane-dark w-full md:w-[260px] shrink-0 flex-col overflow-hidden bg-pine text-white`}>
    ...
    <nav className="mt-8 flex-1 overflow-y-auto px-3">
      ...
      {accounts.map(account => <div key={account.id}>
        ...
        {selectedAccount === account.id && mailboxes.map(box => <button key={box.id} onClick={() => onSelectMailbox(box.id)} className={`ml-8 flex w-[calc(100%-2.5rem)] items-center gap-2 rounded-2xl px-3 py-2 text-left text-xs transition ${selectedMailbox === box.id ? 'bg-white/12 text-white shadow-lift-1' : 'text-white/50 hover:text-white'}`}><FolderIcon role={box.role} />{box.display_name}</button>)}
      </div>)}
      <button onClick={onShowAccounts} className="mt-4 flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-sm text-white/50 transition hover:bg-white/5 hover:text-white"><Plus size={18} />连接邮箱</button>
    </nav>
