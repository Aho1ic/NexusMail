...
export function MessageList({ visible, title, messages, accountMap, selected, unreadCount, markingRead, loading, error, query, cursor, onOpenNav, onMarkViewRead, onRefresh, onQueryChange, onOpen, onLoadMore }: Props) {
  return <section className={`${visible ? 'flex' : 'hidden'} md:flex w-full md:w-[390px] lg:w-[440px] shrink-0 flex-col border-r border-black/5 bg-[#fbfaf6]`}>
    <header className="border-b border-black/5 px-5 pb-4 pt-5">
      <div className="flex items-center justify-between"><button onClick={onOpenNav} className="md:hidden"><Menu size={21} /></button><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-pine/40">Nexus stream</p><h1 className="font-serif text-2xl">{title}</h1></div><div className="flex gap-1"><button onClick={onMarkViewRead} disabled={markingRead || unreadCount === 0} className="icon-button disabled:opacity-35" title={unreadCount ? `将当前视图的 ${unreadCount} 封未读邮件标记为已读` : '当前视图没有未读邮件'} aria-label="全部已读">{markingRead ? <LoaderCircle size={17} className="animate-spin" /> : <CheckCheck size={17} />}</button><button onClick={onRefresh} className="icon-button" aria-label="刷新"><RefreshCw size={17} className={loading ? 'animate-spin' : ''} /></button></div></div>
      <div className="relative mt-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-black/30" size={16} /><input value={query} onChange={event => onQueryChange(event.target.value)} className="w-full rounded-xl border border-black/5 bg-white py-2.5 pl-9 pr-8 text-sm outline-none ring-pine/20 focus:ring-2" placeholder="搜索主题、发件人或正文…" />{query && <button onClick={() => onQueryChange('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-black/30"><X size={15} /></button>}</div>
    </header>
    <div className="flex-1 overflow-y-auto p-2" aria-label="邮件列表">
      ...
      {cursor && <button disabled={loading} onClick={onLoadMore} className="my-3 w-full rounded-xl py-3 text-xs font-semibold text-pine/60 hover:bg-sage/40">{loading ? '加载中…' : '加载更多'}</button>}
    </div>
  </section>
}
